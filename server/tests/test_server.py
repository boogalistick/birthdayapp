...
class TestServerClass:
    ...
    def test_put_another_user(self):
        ...
        resp = req.put("http://127.0.0.1:8080/hello/Delgado", data = '{ "dateOfBirth": "1964-03-17" }', headers={'Content-type': 'application/json'})
        assert 204==resp.status_code

    def test_get_another_user(self):
        """Tests HTTP get request method"""

        resp = req.get(settings.get_base_url()+"/hello/Delgado")
        assert 200==resp.status_code

    def test_put_user_future_date(self):
        """Tests HTTP put request method"""

        resp = req.put("http://127.0.0.1:8080/hello/Delgado", data = '{ "dateOfBirth": "2100-01-01" }', headers={'Content-type': 'application/json'})
        assert 400==resp.status_code

    def test_put_user_with_notexisting_date(self):
        """Tests HTTP put request method 2001 is not a leap year"""

        resp = req.put("http://127.0.0.1:8080/hello/Delgado", data = '{ "dateOfBirth": "2001-02-29" }', headers={'Content-type': 'application/json'})
        assert 400==resp.status_code

    def test_put_wrong_name(self):
        """Tests sending a malformed name"""

        resp = req.put(settings.get_base_url()+"/hello/George2", data = '{ "dateOfBirth": "2014-05-01" }', headers={'Content-type': 'application/json'})
        assert 400==resp.status_code

    def test_put_wrong_path(self):
        """Tests sending a wrong path"""

        resp = req.put(settings.get_base_url()+"/goodbye/George", data = '{ "dateOfBirth": "2014-05-01" }', headers={'Content-type': 'application/json'})
        assert 400==resp.status_code

    def test_put_broken_json(self):
        """Tests sending a malformed json"""

        resp = req.put(settings.get_base_url()+"/hello/George", data = '{ dateOfBirth": "2014-05-01" }', headers={'Content-type': 'application/json'})
        assert 400==resp.status_code

    def test_put_wrong_date(self):
        """Tests sending a malformed date"""

        resp = req.put(settings.get_base_url()+"/hello/George", data = '{ "dateOfBirth": "xxx" }', headers={'Content-type': 'application/json'})
        assert 400==resp.status_code
    ...
    def test_get_missed_user(self):
        ...
        resp = req.get(settings.get_base_url()+"/hello/Angus")
